"""
Training script for Mamba-Small using mamba-ssm.
Uses the same WikiText-103 data pipeline and training hyperparameters as DEM.
NOTE: Requires CUDA - can only run on GPU cluster.
"""
...
try:
    ...
    print("WARNING: mamba-ssm not installed. This script requires CUDA.")
    print("Install with: pip install mamba-ssm (on GPU cluster)")
    raise
...
def main():
    ...
    # Create Mamba-Small model
    print("Creating Mamba-Small model...")
            ...
            if step % args.eval_interval == 0:
                ...
                if step % args.save_interval == 0:
                    checkpoint_path = os.path.join(
                        args.checkpoint_dir,
                        f'mamba_seed{args.seed}_step{step}.pt'
                    )
                    torch.save({
                        'step': step,
    ...
    final_checkpoint_path = os.path.join(
        args.checkpoint_dir,
        f'mamba_seed{args.seed}_final.pt'
    )
    torch.save({
        'step': step,
        'model_state_dict': model.state_dict(),
